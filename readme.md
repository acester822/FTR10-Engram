...
| `/api/dashboard/coherence/link-backfill`                                                                                     | POST                 | One-time legacy link backfill — SQL-only, idempotent `related_to` edges (sim ≥ 0.85, capped)      |
| `/api/dashboard/candidates/process`                                                                                           | POST                 | Drain extraction candidates through the real extraction pipeline (`?limit=N`)                     |
