/*
 ...
 - filename: packages/openmemory-js/src/services/consolidationEngine.ts
...
export class ConsolidationEngine {
  ...
  async getCandidates(
    ...
    // Build the time threshold expression — default to Postgres syntax since OM_STORAGE=postgres
    const isPostgres = (process.env.OM_STORAGE || process.env.OM_STORAGE_BACKEND || "postgres").toLowerCase() !== "sqlite";
...
Synthesized Semantic Memory:`.trim();
 ...
 /**
   ...
   start(): void {
     ...
     this.runConsolidationCycle(realDb, process.env.OM_PG_SCHEMA || "public").catch((err) => {
       console.error("[CodeCortex] Initial consolidation cycle failed:", err);
     });

     const timer = setInterval(() => {
       this.runConsolidationCycle(realDb, process.env.OM_PG_SCHEMA || "public").catch((err) => {
         console.error("[CodeCortex] Scheduled consolidation cycle failed:", err);
       });
     }, intervalMs);
  ...
  async runConsolidationCycle(
    ...
    if (result.consolidated > 0) {
      console.log(
        `[CodeCortex] ✅ Consolidation complete. ${result.consolidated} groups → ${result.deleted} memories consolidated into semantic memory: ${result.newMemoryId}`,
      );
    } else {
      console.log("[CodeCortex] No consolidation candidates found.");
    }
    return result;
  }
}
