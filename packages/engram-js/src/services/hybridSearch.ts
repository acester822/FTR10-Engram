...
export class HybridSearch {
  ...
  async search(query: string, limit: number = 10): Promise<SearchResult[]> {
    ...
    const vectorResults = await this.db.query(
      `with vec as (
         select id, content, sector, importance_score, importance_tier,
                max(vector_score) as vector_score
         from (
           select m.id, m.content, m.sector, m.importance_score, m.importance_tier,
         ...
         group by id, content, sector, importance_score, importance_tier
       )
       select * from vec
