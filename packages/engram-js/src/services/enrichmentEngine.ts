  ...
  for (const cand of candidates) {
    ...
    try {
      const v = await callJudge(
        `You are a strict enrichment validator. Given ORIGINAL, ENRICHED, and SOURCES, judge whether ENRICHED preserves the original and adds ONLY facts supported by the sources. REJECT (score 0) if: (a) any addition imports facts about a DIFFERENT project, tool, or codebase than the original memory — including a DIFFERENT NAMED product that discusses the same concepts (mem0 is not Engram; Langfuse is not this system; a tutorial about another tool is not this project); (b) any addition is not directly supported by a source; (c) the enrichment drifts off-topic or invents specifics; (d) the enrichment adds ONLY a source tag, a generic tutorial, generic best practice, or vendor defaults that do not pertain specifically to THIS memory's fact. Respond ONLY with JSON: {"score": <0.0-1.0>, "reason": "<one sentence>"} where 0 = terrible and 1 = perfect.`,
