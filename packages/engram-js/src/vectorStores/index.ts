...
export function getVectorStoreConfig(
  ...
  const kind = normalizeVectorStoreKind(source.EG_VECTOR_STORE);
  const collection = str(source.EG_VECTOR_COLLECTION, "engram_memories");
  const timeout_ms = num(source.EG_VECTOR_TIMEOUT_MS, 10000);
  const endpoint =
    source.EG_VECTOR_URL ||
    source[`EG_${kind.toUpperCase()}_URL`] ||
    source[`${kind.toUpperCase()}_URL`] ||
    (kind === "valkey" ? source.REDIS_URL : undefined) ||
    (kind === "pinecone" ? source.PINECONE_INDEX_HOST : undefined);
  const api_key =
    source.EG_VECTOR_API_KEY ||
    source[`EG_${kind.toUpperCase()}_API_KEY`] ||
