/*
 ...
 - filename: packages/engram-js/src/database/migrate.ts
...
async function run_durable_schema_migration(pool: Pool): Promise<void> {
  const schema = process.env.EG_PG_SCHEMA || "public";
  const vectorDim = process.env.EG_VEC_DIM
    ? +process.env.EG_VEC_DIM
    : env.vec_dim;
...
export async function run_migrations() {
  ...
  const pool = new Pool(
    build_pg_pool_config(process.env.EG_PG_DB || "engram"),
  );
