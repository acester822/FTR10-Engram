...
- **Repo baseline index (v4.7.0-repo-index)**: Repos tab — URL clone or local path → T1/T2 structural miners (zero LLM) + git history (commits/mutations/reverts → "remember what broke"); repo-identity tagged, redacted, audited, re-index supersedes; auto-refresh fingerprint pass (mtime + git HEAD); working-location tip in cognitive-context
- **Mind Map v2**: `/api/memory-graph` — stratified sector sampling (all memory types visible), real typed edges (edges table: part_of/related_to/derives_from) + cosine proximity, zoom/pan/node-drag, labels, stats bar, per-type legend with counts
