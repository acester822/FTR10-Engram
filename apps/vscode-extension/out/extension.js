...
const cursor_1 = require("./writers/cursor");
const claude_1 = require("./writers/claude");
const windsurf_1 = require("./writers/windsurf");
const copilot_1 = require("./writers/copilot");
const codex_1 = require("./writers/codex");
const DashboardPanel_1 = require("./panels/DashboardPanel");
const diff_1 = require("./utils/diff");
function isCodeServer() {
    try {
        const remoteName = vscode.env.remoteName || '';
        if (remoteName && remoteName.toLowerCase().includes('codeserver'))
            return true;
    }
    catch { /* not available */ }
    return false;
...
function activate(context) {
    ...
    const dashboard_cmd = vscode.commands.registerCommand('openmemory.dashboard', () => { DashboardPanel_1.DashboardPanel.createOrShow(context.extensionUri); });
    ...
    const save_listener = vscode.workspace.onDidSaveTextDocument((doc) => {
        if (is_enabled && is_tracking && doc.uri.scheme === 'file') {
            ...
            let contentToSend = "";
            if (oldContent) {
                const diff = (0, diff_1.generateDiff)(oldContent, newContent, doc.uri.fsPath);
...
async function auto_link_all() {
    ...
    try {
        const configs = [];
        configs.push(await (0, cursor_1.writeCursorConfig)(backend_url, api_key, use_mcp, mcp_server_path));
        configs.push(await (0, claude_1.writeClaudeConfig)(backend_url, api_key, use_mcp, mcp_server_path));
        configs.push(await (0, windsurf_1.writeWindsurfConfig)(backend_url, api_key, use_mcp, mcp_server_path));
        configs.push(await (0, copilot_1.writeCopilotConfig)(backend_url, api_key, use_mcp, mcp_server_path));
        configs.push(await (0, codex_1.writeCodexConfig)(backend_url, api_key, use_mcp, mcp_server_path));
...
async function show_menu() {
    ...
    switch (choice.action) {
        case 'dashboard':
            vscode.commands.executeCommand('openmemory.dashboard');
            break;
        case 'pause':
            is_tracking = false;
        ...
        case 'query':
            ...
            break;
        case 'add':
            vscode.commands.executeCommand('openmemory.addToMemory');
            break;
        case 'note':
...
async function show_quick_setup() {
    ...
    switch (choice.action) {
        ...
        case 'url':
            ...
            break;
        case 'docs':
            vscode.env.openExternal(vscode.Uri.parse('https://github.com/CaviraOSS/OpenMemory'));
            break;
        case 'test':
            update_status_bar('connecting');
            const connected = await check_connection();
...
function format_memories(memories) {
    ...
    if (memories.length === 0)
        return out + 'No relevant memories found.\n';
...
function format_patterns(patterns) {
    ...
    if (patterns.length === 0)
        return out + 'No patterns detected.\n';
...
async function gatherChatLocalContext() {
    let context = '';
    const editor = vscode.window.activeTextEditor;
    if (editor) {
    ...
    try {
        ...
        if (workspaceFolder) {
            const branch = (0, child_process_1.execSync)('git branch --show-current', { cwd: workspaceFolder, encoding: 'utf-8' }).trim();
...
function renderDynamicCognitiveTrace(stream, trace) {
...
function registerChatParticipant(context) {
            ...
            while (true) {
                ...
                for (const msg of chunks) {
                    ...
                    if (eventType === 'message' && eventData !== '[DONE]') {
                        ...
                        try {
                            dynamicTrace = JSON.parse(eventData);
                        }
                        catch (e) {
                            console.error('[OpenMemory] Failed to parse trace data:', e);
                        }
                    }
                }
            }
    ...
    const iconUri = vscode.Uri.file(path.join(context.extensionPath, 'icon.png'));
