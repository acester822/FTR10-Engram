...
**Verification (all live):** fixture repo with a genuine `Revert "fix: adjust unit label"` commit → 4 files + 3 commits + **1 revert** + anchor = 9 memories, revert recall ranks #1 (0.695) for "what broke"; `.env.example` token redacted (0 rows contain it); **idempotency**: re-index twice → exactly 9 active, each with 1 superseded predecessor, zero duplicates; **cross-repo isolation**: two repos → 0 cross-repo edges, identity is the path (a same-named repo cannot mingle); URL clone of FTR10-Engram → 213 files + 109 commits (clone depth = commit cap so history exists to mine); **tip**: silent ×2, fires once on 3rd call, silent after; indexed repo suppresses it. All 8 commits pushed (`c108de50e` → `13e0e73d8` + follow-ups).
