...
**IMPLEMENTED 2026-08-08 (v4.7.0-repo-index)** — user decisions honored: T1 + T2 both, Docker volumes approved, server-side tip, gated doc-LLM.

**Shipped:** `repos` table (schema bump `4.7.0-repo-index`); `repoStore.ts` (CRUD); `repoMiner.ts` (T1 line-based + T2 tree-sitter web-wasm, per-language grammars vendored from npm — the reference tool's proven path, zero-LLM); `gitMiner.ts` (commit history, per-file mutations, **revert detection → bi-temporal "remember what broke" memories**); `repoIndexer.ts` (walker with caps/skip rules, redaction via the shared `redactSecrets`, per-file supersede on re-index with full audit, run-locked, progress, kind-aware dedupe); `dashboard/repos` route (index/list/progress/reindex/delete/recall-test); Repos tab in the GUI (add by URL or local path, progress bar, reindex/delete, repo-scoped recall test); Settings → Repos group (`EG_REPOS_ROOT`, `EG_REPO_MAX_FILES`, `EG_REPO_MAX_FILE_BYTES`, `EG_REPO_MAX_COMMITS`, `EG_REPO_CLONE_DEPTH`, `EG_REPO_TIP_ENABLED`); cognitive-context **working-location tip** (Redis session counter, fires once on the 3rd injection for an unindexed project, toggleable, never automatic); compose volumes (`./repos:/data/repos` writable clones + `/home/ftr/Apps:/data/repos-local:ro` read-only local paths, host-path translation in `resolveSource`); image: `git` + `ca-certificates` + `safe.directory *` (host-mounted repo ownership).

**Verification (all live):** fixture repo with a genuine `Revert "fix: adjust unit label"` commit → 4 files + 3 commits + **1 revert** + anchor = 9 memories, revert recall ranks #1 (0.695) for "what broke"; `.env.example` token redacted (0 rows contain it); **idempotency**: re-index twice → exactly 9 active, each with 1 superseded predecessor, zero duplicates; **cross-repo isolation**: two repos → 0 cross-repo edges, identity is the path (a same-named repo cannot mingle); URL clone of FTR10-Engram → 213 files + 109 commits (clone depth = commit cap so history exists to mine); **tip**: silent ×2, fires once on 3rd call, silent after; indexed repo suppresses it. **Auto-refresh** (follow-up): new file added to a repo → `needs-refresh` flips false→true → refresh pass re-indexes (new file lands in the store), second refresh = 0 changed (idempotent), delete supersedes 6. All commits pushed (`c108de50e` → `f0c98d696`).
