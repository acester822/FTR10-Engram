...
* **Configuration:** Standalone environment variables (`EG_CONSOLIDATION_MODEL`, `EG_EXTRACTION_MODEL`) with hardcoded fallbacks (`qwen2.5:14b` for consolidation, `qwen2.5:7b` for extraction).
...
| :--- | :--- | :--- |
| **Extraction** (Parsing transcripts into JSON) | `qwen2.5:7b` | The absolute sweet spot for CPU extraction. It has phenomenal instruction-following capabilities, reliably outputs strict JSON, and runs in 1–3 seconds on a 28-core Xeon. |
